...
def format_coco(chip_dfs: Dict, chip_width: int, chip_height: int, multi=False):
    ...
    if multi:
        categories= [
                        {"supercategory": "AgriculturalFields","id": 1,"name": "springcereal"},
                        {"supercategory": "AgriculturalFields","id": 2,"name": "wintercereal"},
                        {"supercategory": "AgriculturalFields","id": 3,"name": "maize"},
                        {"supercategory": "AgriculturalFields","id": 4,"name": "grassland"},
                        {"supercategory": "AgriculturalFields","id": 5,"name": "other"},
                   ]
    else:
    ...
    for chip_name in chip_dfs.keys():
        ...
        for _, row in chip_dfs[chip_name]['chip_df'].iterrows():
            ...
            if multi:
                annotation["category_id"] = row['r_lc_id']
                annotation["mycategory_name"] = row['r_lc_name']
            else:
                annotation["old_multiclass_category_name"] = row['r_lc_name']
                annotation["old_multiclass_category_id"] = row['r_lc_id']
...
def move_coco_val_images(inpath_train_folder, val_chips_list):
    ...
    print('val path: ',outpath_val_folder)
