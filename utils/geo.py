...
def print_len(df ,caller):
    print(rf'caller:{caller}, nr_polys: {len(df)}')
    return df
...
def cut_chip_geometries(vector_df, raster_width, raster_height, raster_transform, chip_width=128, chip_height=128, first_n_chips=None):
    ...
    for i, (chip_window, chip_transform, chip_poly) in enumerate(tqdm(generator_window_bounds)):
        ...
        tt = len(chip_df)
        chip_df = chip_df[chip_df.geometry.area > 5000]  #5000 sqm in UTM
        ttt = len(chip_df)
        deleted_chips += (tt - ttt)
        ...
        all_chip_dfs[chip_name] = {'chip_df': chip_df,
                                   ...
                                   'chip_poly': chip_poly}
    print_sth(f'deleted small fields: {deleted_chips}')
    return all_chip_dfs
